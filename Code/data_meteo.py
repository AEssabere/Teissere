...
def get_url(mois, annee):
    """ Recupere l'URL de la page a recuperer
        ...
        Sortie :
            URL du mois et de l'annee souhaites
    """
...
def supp_le(string):
    """ Supprime l'element "le"
        ...
        Sortie :
            dans le cas ou string contient "le", on retourne tout ce qui se trouve avant "le"
...
def remplir_dict(temp, station, dic, departement):
    """ Verifie si le numero de departement recupere est bien dans la liste des departements de 
        la France metropolitaine, si c'est le cas, on associe la temperature au numero du
        ...
        Entree :
            temp : liste des temperatures moyennes du mois (ttm), des moyennes des temperatures
                maximales du mois (txm) ou des temperatures maximales extremes du mois (txx)
        ...
        Sortie : 
            void
    """
    string = "".join(departement)   # met dans une seule chaine de caracteres tous les departements
...
def cal_moy(dic):
    """ Calcule la moyenne des temperatures des stations appartenant aux memes departements
        Entree :
            Dictionnaire qui contient les moyennes de chaque station
        Sortie :
            une liste des moyennes calculees
    """
    list_moyenne = []
...
def creer_dft(departement):
    """ Cree la transposee d'un dataframe
        Entree :
            Liste des departements
        Sortie :
            la transposee du dataframe
    """
    df = pd.DataFrame(columns=departement)
    return df.T

def get_data(deb_annee, fin_annee, deb_mois, fin_mois):
    """ Recupere les donnees meteorologiques (temperatures moyennes, maximales extremes et
        les moyennes des temperatures maximales du mois) 
        ainsi que les departements et exporte toutes ces donnees dans des fichiers csv.
        Entree :
    ...
    # le dataframe de la moyenne des temperatures du mois
    df_tmm = creer_dft(departement)

    # le dataframe des moyennes des temperatures maximales du mois
    df_txm = creer_dft(departement)

    # le dataframe des temperatures maximales extremes du mois
    ...
    for annee in range(deb_annee, fin_annee+1):
        for mois in range(deb_mois, fin_mois+1):
            ...
            # ajout des moyennes des temperatures calculees par departement dans les dataframes
            df_tmm[date] = [date] + cal_moy(dic_tmm)
            df_txm[date] = [date] + cal_moy(dic_txm)
            df_txx[date] = [date] + cal_moy(dic_txx)

    # Mettre en indice les dates pour chacuns des dataframes
    df_tmm = df_tmm.T
    ...
    # convertir les dataframes en quatre fichiers csv
